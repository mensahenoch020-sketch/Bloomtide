...
// Keep admin auth simple and reliable on Railway. A restart signs the admin out,
// but avoids proxy/session-store issues while the site runs as a single service.
app.use(session({
...
async function initDb(){
  ...
  // Railway variables are the source of truth for the admin login.
  if(process.env.ADMIN_EMAIL&&process.env.ADMIN_PASSWORD){
    const hash=await bcrypt.hash(process.env.ADMIN_PASSWORD,12);
    await pool.query(`insert into admins(email,password_hash) values($1,$2)
      on conflict(email) do update set password_hash=excluded.password_hash`,[process.env.ADMIN_EMAIL.toLowerCase().trim(),hash]);
    console.log('Bloomtide admin credentials synchronized from Railway');
  }
  ...
  const job={
    ...
    salary_range:'',
    description:'Bloomtide is seeking a professional and dependable Customer Service Representative to support clients throughout the appraisal process. This role responds to inquiries, helps clients provide property information, coordinates communication around inspections and appraisal requests, shares status updates, and ensures each interaction reflects Bloomtide’s standard of clarity and professionalism.',
  ...
  if(existing.rowCount){
    await pool.query(`update jobs set location=$1,department=$2,employment_type=$3,salary_range=$4,description=$5,requirements=$6,published=true,closing_date=null where id=$7`,[job.location,job.department,job.employment_type,job.salary_range,job.description,job.requirements,existing.rows[0].id]);
  }else{
    await pool.query(`insert into jobs(title,location,department,employment_type,salary_range,description,requirements,published) values($1,$2,$3,$4,$5,$6,$7,true)`,[job.title,job.location,job.department,job.employment_type,job.salary_range,job.description,job.requirements]);
  }
  console.log('Customer Service Representative job is published');
}
...
app.get('/api/address-suggestions',asyncRoute(async(req,res)=>{
  const q=safe(req.query.q||'',180);
  ...
  const r=await fetch(url,{headers:{'User-Agent':'Bloomtide/1.0 (charlierob1225@gmail.com)','Accept-Language':'en-US,en'}});
  if(!r.ok) return res.json([]);
  const data=await r.json();
  res.json(data.map(x=>({label:x.display_name,value:x.display_name})).slice(0,5));
}));

app.post('/api/appraisals',asyncRoute(async(req,res)=>{
  const {name,email,phone,property_address,property_type,purpose,preferred_inspection_date,message}=req.body;
  if(!name||!email||!phone||!property_address||!property_type||!purpose)return res.status(400).json({error:'Please complete all required fields.'});
...
app.post('/api/applications',asyncRoute(async(req,res)=>{
  const {job_id,name,email,phone,linkedin_url,resume_url,cover_letter}=req.body;
  if(!job_id||!name||!email)return res.status(400).json({error:'Job, name and email are required.'});
  const r=await pool.query(`insert into job_applications(job_id,name,email,phone,linkedin_url,resume_url,cover_letter) values($1,$2,$3,$4,$5,$6,$7) returning id,status`,[job_id,safe(name,150),safe(email,200),safe(phone,80),safe(linkedin_url,500),safe(resume_url,1000),safe(cover_letter,10000)]);
  res.status(201).json(r.rows[0]);
}));
...
app.post('/api/admin/login',asyncRoute(async(req,res)=>{
  ...
  if(!r.rowCount||!(await bcrypt.compare(password,r.rows[0].password_hash)))return res.status(401).json({error:'Invalid admin email or password.'});
  req.session.regenerate(err=>{
    if(err) return res.status(500).json({error:'Could not start admin session.'});
    req.session.adminId=r.rows[0].id;
    req.session.adminEmail=r.rows[0].email;
    req.session.save(saveErr=>saveErr?res.status(500).json({error:'Could not save admin session.'}):res.json({ok:true,email:r.rows[0].email}));
  });
}));
...
app.patch('/api/admin/appraisals/:id',requireAdmin,asyncRoute(async(req,res)=>{
  const status=safe(req.body.status||'',80);
  const rawFee=req.body.fee;
  const fee=rawFee===''||rawFee==null?null:Number(rawFee);
  if(!allowedStatuses.has(status))return res.status(400).json({error:'Invalid appraisal status.'});
  if(fee!==null&&(!Number.isFinite(fee)||fee<0))return res.status(400).json({error:'Enter a valid fee.'});
  const r=await pool.query('update appraisal_requests set status=$1,fee=$2 where id=$3 returning id,status,fee',[status,fee,req.params.id]);
  if(!r.rowCount)return res.status(404).json({error:'Appraisal request not found.'});
  res.json(r.rows[0]);
}));
...
app.use(express.static(path.join(__dirname),{extensions:['html'],maxAge:0,etag:true}));
app.get('*',(req,res)=>res.sendFile(path.join(__dirname,'index.html')));
app.use((err,req,res,next)=>{console.error(err);if(req.path.startsWith('/api/'))return res.status(500).json({error:'Something went wrong. Please try again.'});next(err);});
