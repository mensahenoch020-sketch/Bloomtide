...
async function initDb(){
  await pool.query(`
    ...
    alter table job_applications add column if not exists resume_filename text;
    alter table job_applications add column if not exists resume_mimetype text;
...
app.post('/api/applications',uploadResume,asyncRoute(async(req,res)=>{const {job_id,name,email,phone,linkedin_url,cover_letter}=req.body;if(!job_id||!name||!email)return res.status(400).json({error:'Job, name and email are required.'});const jobRow=await pool.query('select title from jobs where id=$1',[job_id]);const f=req.file;const r=await pool.query(`insert into job_applications(job_id,name,email,phone,linkedin_url,cover_letter,resume_filename,resume_mimetype,resume_data) values($1,$2,$3,$4,$5,$6,$7,$8,$9) returning id,status`,[job_id,safe(name,150),safe(email,200),safe(phone,80),safe(linkedin_url,500),safe(cover_letter,10000),f?safe(f.originalname,255):null,f?f.mimetype:null,f?f.buffer:null]);res.status(201).json(r.rows[0]);notify(`New Bloomtide job application: ${jobRow.rows[0]?.title||'Job'}`,`New job application received.\n\nPosition: ${jobRow.rows[0]?.title||'Unknown'}\nName: ${name}\nEmail: ${email}\nPhone: ${phone||'Not provided'}\nLinkedIn: ${linkedin_url||'Not provided'}\nResume: ${f?f.originalname+' (attached to this email)':'Not provided'}\nCover letter: ${cover_letter||'None'}\n\nSign in to the admin dashboard to review this application.`,f?[{filename:f.originalname,content:f.buffer,contentType:f.mimetype}]:undefined);}));
...
app.patch('/api/admin/jobs/:id',requireAdmin,asyncRoute(async(req,res)=>{if(typeof req.body.published==='boolean'){if(req.body.published)await pool.query('update jobs set published=$1,closing_date=null where id=$2',[req.body.published,req.params.id]);else await pool.query('update jobs set published=$1 where id=$2',[req.body.published,req.params.id]);}res.json({ok:true});}));
