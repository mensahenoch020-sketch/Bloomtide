...
if(!SESSION_SECRET) console.warn('SESSION_SECRET is not set. Add it in Railway before production use.');

// Railway terminates HTTPS at its proxy. Trust it so secure session cookies work correctly.
...
app.use(session({
  ...
  secret:SESSION_SECRET||'development-only-secret',
  resave:false,
...
async function initDb(){
  ...
  if(process.env.ADMIN_EMAIL&&process.env.ADMIN_PASSWORD){
    const existing=await pool.query('select id,password_hash from admins where lower(email)=lower($1)',[process.env.ADMIN_EMAIL]);
    ...
    if(!existing.rowCount){
      ...
      console.log('Bloomtide admin password synchronized from Railway');
    }
  }

  const existingJob=await pool.query(`select id from jobs where lower(title)=lower('Customer Service Representative') limit 1`);
  if(!existingJob.rowCount){
    await pool.query(`insert into jobs(title,location,department,employment_type,description,requirements,published)
      values($1,$2,$3,$4,$5,$6,true)`,[
      'Customer Service Representative',
      'Remote · United States',
      'Client Services',
      'Full-time',
      'Bloomtide is seeking a professional, dependable Customer Service Representative to support clients throughout the appraisal process. You will respond to inquiries, help clients submit property information, coordinate communication around inspections and appraisal requests, provide status updates, and ensure every interaction reflects Bloomtide’s standard of clarity and professionalism.',
      'Strong written and verbal communication skills\nProfessional and courteous client service\nComfort working with email, phone and web-based systems\nStrong organization and attention to detail\nAbility to manage multiple client requests responsibly\nReal estate, appraisal, mortgage or related customer-service experience is helpful but not required',
    ]);
    console.log('Customer Service Representative job published');
  }
}
...
app.post('/api/appraisals',asyncRoute(async(req,res)=>{
  ...
  if(!name||!email||!phone||!property_address)return res.status(400).json({error:'Please complete all required fields.'});
...
app.post('/api/admin/login',asyncRoute(async(req,res)=>{
  ...
  req.session.adminId=r.rows[0].id;req.session.adminEmail=r.rows[0].email;
  req.session.save(err=>err?res.status(500).json({error:'Could not start admin session.'}):res.json({ok:true,email:r.rows[0].email}));
}));
...
app.patch('/api/admin/appraisals/:id',requireAdmin,asyncRoute(async(req,res)=>{const {status,fee}=req.body;await pool.query('update appraisal_requests set status=$1,fee=$2 where id=$3',[safe(status,80),fee===''||fee==null?null:Number(fee),req.params.id]);res.json({ok:true});}));
